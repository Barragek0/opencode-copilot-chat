...
Total `src/` ≈ **14,782 lines** across ~70 files. Grouped by domain (folders) then root-level utilities.

| Domain                      | Folder / Files                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | LoC    | Owns                                                                                                                                                                                        | Key contracts                                                                                                                     |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **Provider**                | `src/provider/` — `OpenCodeProvider.ts` (1,215), `definitions.ts` (275), `messages.ts` (422), `settings.ts` (199), `visionProxy.ts` (322), `tokens.ts` (106)                                                                                                                                                                                                                                                                                                                                                             | ~2,539 | The `OpenCodeProvider` class implementing `LanguageModelChatProvider`, the `PROVIDERS` table (4 vendor definitions), message↔wire conversion, per-model settings/schema, vision proxy relay | `convertMessage()` / `normalizeMessages()` / `trimOldImagesFromHistoryInPlace()` / `modelLimits()` / `modelConfigurationSchema()` |
...
| **Models (metadata)**       | `src/models/` — `metadata.ts` (646), `metadataFetcher.ts` (102), `modelLimits.ts` (52), `modelCapabilities.ts` (16), `modelNames.ts` (29), `pricing.ts` (88)                                                                                                                                                                                                                                                                                                                                                             | ~933   | models.dev live metadata + bundled fallback snapshot, limit/capability resolution, pricing                                                                                                  | Live fetch may fail → bundled snapshot MUST exist                                                                                 |
| **Usage**                   | `src/usage/` — `dashboard.ts` (1,317), `tracker.ts` (985), `history.ts` (373), `usage.ts` (146), `goUsageSync.ts` (127), `formatting.ts` (126), `usageProfile.ts` (74), `pricing.ts` (52)                                                                                                                                                                                                                                                                                                                                | ~3,200 | Go usage tracker, per-profile tracking, CLI SQLite history reader, server-usage sync, status bar + usage webview + quick-pick                                                               | Server meters authoritative for Session/Weekly/Monthly; device-local for Today/Yesterday                                          |
...
4. For a usage change: read `src/usage/tracker.ts` + `dashboard.ts` (the two biggest files in the domain).

### Step 3 — Check Cross-Module Dependencies

See the dependency graph in Section 3. The heaviest fan-in is `src/provider/OpenCodeProvider.ts` (imports 15+ modules) — any change to a module it imports must be checked against it.
...
flowchart TD
    ...
    subgraph Usage
        TRK[usage/tracker.ts]
        DSH[usage/dashboard.ts]
...
| File                               | LoC   | Note                                                                                                                                                                                                                                                                                                                              |
| ---------------------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/usage/dashboard.ts`           | 1,317 | Self-contained SPA webview (~600-line inline HTML template: 6 metric tabs Spend/Requests/Tokens/Models/Suggested/Approved, ring cards, stat chips, vanilla-JS chart engine with cursor-follow tooltip, CSP `default-src 'none'`) + status bar + SVG tooltip builder + profile management — candidate for `usage/webview.ts` split |
| `src/provider/OpenCodeProvider.ts` | 1,215 | Provider class — candidate: extract fetch/diagnostics                                                                                                                                                                                                                                                                             |
| `src/usage/tracker.ts`             | 985   | Tracker + SQLite + UI — mostly already split from the old god file                                                                                                                                                                                                                                                                |
| `src/models/metadata.ts`           | 646   | Snapshot normalization + limits helpers                                                                                                                                                                                                                                                                                           |
