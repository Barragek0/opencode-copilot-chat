...
void describe("calculateModelLimits", () => {
  void it("uses a conservative registration budget when prompt size is unknown", () => {
  ...
  void it("caps output to the context remaining after the prompt and safety margin", () => {
    const limits = calculateModelLimits(metadata, { promptTokens: 70_000 });

    assert.equal(limits.maxOutputTokens, 21_600);
  });

  void it("never restores a 4K minimum that would overflow a nearly full context", () => {
    const limits = calculateModelLimits(metadata, { promptTokens: 99_990 });

    assert.equal(limits.maxOutputTokens, 1);
  });

  void it("honors context and output overrides without exceeding either", () => {
    const limits = calculateModelLimits(metadata, {
      contextSize: 50_000,
  ...
  void it("keeps the issue #109 DeepSeek request below the real context limit", () => {
