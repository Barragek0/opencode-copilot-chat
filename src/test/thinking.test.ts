 ...
 */
void describe("buildThinkingPayload — kimi-k2.7-code (issue #25)", () => {
  void it("always emits { type: 'enabled', keep: 'all' } even when thinking.kimi is 'off'", () => {
  ...
  void it("emits { type: 'enabled', keep: 'all' } when thinking.kimi is 'on'", () => {
    const payload = buildThinkingPayload("kimi-k2.7-code", { ...defaultSettings, kimi: "on" });
    assert.deepEqual(payload, { thinking: { type: "enabled", keep: "all" } });
  });

  void it("matches kimi-k2.7-code-highspeed variant too (same model, faster output)", () => {
    const payload = buildThinkingPayload("kimi-k2.7-code-highspeed", defaultSettings);
    assert.deepEqual(payload, { thinking: { type: "enabled", keep: "all" } });
  });
});

void describe("buildThinkingPayload — regression safety for other kimi models", () => {
  void it("kimi-k2.6 with kimi='off' emits { type: 'disabled' } (still accepts disabled)", () => {
  ...
  void it("kimi-k2.5 with kimi='off' emits { type: 'disabled' }", () => {
    const payload = buildThinkingPayload("kimi-k2.5", { ...defaultSettings, kimi: "off" });
    assert.deepEqual(payload, { thinking: { type: "disabled" } });
  });
});

void describe("buildThinkingPayload — other families unchanged", () => {
  void it("deepseek with 'off' emits empty object (no reasoning_effort)", () => {
  ...
  void it("glm with 'off' emits { type: 'disabled' }", () => {
    const payload = buildThinkingPayload("glm-5", { ...defaultSettings, glm: "off" });
    assert.deepEqual(payload, { thinking: { type: "disabled" } });
  });

  void it("qwen with 'off' emits enable_thinking: false", () => {
 ...
 */
void describe("buildFamilyThinkingSchema — kimi-k2.7-code picker", () => {
  void it("exposes a single 'on' option with 'Always On (K2.7)' label", () => {
  ...
  void it("mentions the Moonshot API constraint in the description", () => {
    const schema = buildFamilyThinkingSchema("kimi-k2.7-code");
    assert.ok(schema, "expected schema to be defined");
    const reasoningEffort = schema.properties.reasoningEffort as Record<string, unknown>;
...
void describe("buildFamilyThinkingSchema — other kimi models keep off/on", () => {
  void it("kimi-k2.6 exposes both 'off' and 'on'", () => {
  ...
  void it("kimi-k2.5 exposes both 'off' and 'on'", () => {
 ...
 */
void describe("applyRequestThinkingOverride — kimi-k2.7-code defensive force-on", () => {
  void it("forces kimi='on' even when override requests 'off'", () => {
  ...
  void it("forces kimi='on' even when override requests 'on' (no-op but explicit)", () => {
    const result = applyRequestThinkingOverride("kimi-k2.7-code", defaultSettings, {
      reasoningEffort: "on",
    });
    assert.equal(result.kimi, "on");
  ...
  void it("forces kimi='on' when override is empty (defensive against stale cache)", () => {
    const result = applyRequestThinkingOverride("kimi-k2.7-code", defaultSettings, {});
    assert.equal(result.kimi, "on");
  });
});

void describe("applyRequestThinkingOverride — other kimi models respect override", () => {
  void it("kimi-k2.6 respects 'off' override", () => {
  ...
  void it("kimi-k2.6 respects 'on' override", () => {
    const result = applyRequestThinkingOverride("kimi-k2.6", defaultSettings, {
      reasoningEffort: "on",
    });
    assert.equal(result.kimi, "on");
...
void describe("thinkingFamily — detection", () => {
  void it("classifies kimi-k2.7-code as 'kimi'", () => {
  ...
  void it("returns null for unknown prefixes", () => {
 ...
 */
void describe("buildThinkingPayload — GLM with effort values (issue #61)", () => {
  void it("glm-5.2 with glm='high' emits reasoning_effort: 'high'", () => {
    const payload = buildThinkingPayload("glm-5.2", { ...defaultSettings, glm: "high" });
    assert.deepEqual(payload, { reasoning_effort: "high" });
  });

  void it("glm-5.2 with glm='max' emits reasoning_effort: 'max'", () => {
    const payload = buildThinkingPayload("glm-5.2", { ...defaultSettings, glm: "max" });
    assert.deepEqual(payload, { reasoning_effort: "max" });
  });

  void it("glm-5.2 with glm='off' emits thinking disabled", () => {
    const payload = buildThinkingPayload("glm-5.2", { ...defaultSettings, glm: "off" });
    assert.deepEqual(payload, { thinking: { type: "disabled" } });
  });

  void it("glm-5 (toggle-only) with glm='high' sends reasoning_effort (gateway resolves)", () => {
    const payload = buildThinkingPayload("glm-5", { ...defaultSettings, glm: "high" });
    assert.deepEqual(payload, { reasoning_effort: "high" });
  });

  void it("glm-5 (toggle-only) with glm='off' emits thinking disabled", () => {
    const payload = buildThinkingPayload("glm-5", { ...defaultSettings, glm: "off" });
    assert.deepEqual(payload, { thinking: { type: "disabled" } });
  });
});

void describe("buildFamilyThinkingSchema — GLM 5.2 with reasoning_options metadata", () => {
  void it("exposes off, high, max when reasoning_options has effort values", () => {
    const metadata = {
      reasoning: true,
  ...
  void it("falls back to off/high/max for GLM models without reasoning_options (no invalid 'on')", () => {
...
void describe("applyRequestThinkingOverride — GLM with effort values (issue #61)", () => {
  void it("accepts 'high' override for glm-5.2", () => {
  ...
  void it("accepts 'max' override for glm-5.2", () => {
    const result = applyRequestThinkingOverride("glm-5.2", defaultSettings, {
      reasoningEffort: "max",
    });
  ...
  void it("accepts 'off' override for glm-5.2", () => {
    const result = applyRequestThinkingOverride("glm-5.2", defaultSettings, {
      reasoningEffort: "off",
    });
  ...
  void it("rejects invalid values like 'on' and 'medium' for glm", () => {
